function importQuestion() {
    ...
    if (metabaseQuestionNum != 'cancel' && !isNaN(metabaseQuestionNum)) {
      ...
      try{
        Core.getQuestionAndFillSheet({"id": metabaseQuestionNum});
  ...
  function importAllQuestions() {
    ...
    var result = ui.alert(
      'Isso ira importar as consultas para abas contendo "(metabase/NUMERO_DA_CONSULTA)". Após a barra o numero da consulta no Metabase (exemplo: "(metabase/152)").',
      ui.ButtonSet.YES_NO);
    ...
    if (result == ui.Button.YES) {
      ...
      var go = ui.alert(
          'Importar ' + questions.length + ' consulta(s): ' + questionNumbers.join(', ') + '. Continuar?',
          ui.ButtonSet.YES_NO);
      ...
      if (go == ui.Button.YES) {
        ...
        for (var i = 0; i < questions.length; i++) {
          ...
          try{
            Core.getQuestionAndFillSheet({"id": metabaseQuestionNum, "sheet": sheetName});
